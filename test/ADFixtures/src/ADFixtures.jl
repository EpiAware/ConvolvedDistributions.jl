...
function scenarios(; with_reference::Bool = false, category::Symbol = :marginal)
    ...
    _push!(
        "Convolved Normal+Normal analytical",
        (θ, obs) -> sum(
            x -> logpdf(
                convolved(
                    Normal(θ[1], θ[2]), Normal(0.0, 1.0)
                ), x
            ), obs
        ),
    ...
    _push!(
        "Convolved Gamma+LogNormal numerical",
        (θ, obs) -> sum(
            x -> logpdf(
                convolved(
    ...
    # scenario above.
    _push!(
        "Convolved Gamma+LogNormal custom GaussLegendre payload numerical",
        (θ, obs) -> sum(
            x -> logpdf(
                convolved(
                    Gamma(θ[1], θ[2]), LogNormal(0.5, 0.4);
    ...
    _push!(
        "Convolved LogNormal+Gamma numerical",
        (θ, obs) -> sum(
            x -> logpdf(
                convolved(
    ...
    # tracked storage (#44).
    _push!(
        "Convolved Gamma+LogNormal batched logpdf wrt params",
        (θ, obs) -> sum(
            logpdf(
                convolved(
                    Gamma(θ[1], θ[2]), LogNormal(0.5, 0.4)
    ...
    _push!(
        "Convolved Gamma+LogNormal batched logpdf wrt points",
        (x, _obs) -> sum(
            logpdf(
                convolved(
    ...
    # uniform.
    _push!(
        "Convolved Gamma+Normal mean+var moments",
    ...
    cdf_obs = [0.5, 1.5, 3.0]
    _push!(
        "Convolved Gamma+Uniform closed-form logcdf",
        (θ, xs) -> sum(
            x -> logcdf(
    ...
    _push!(
        "Convolved LogNormal+Uniform closed-form logcdf",
        (θ, xs) -> sum(
            x -> logcdf(
    ...
    _push!(
        "Convolved Weibull+Uniform closed-form logcdf",
        (θ, xs) -> sum(
            x -> logcdf(
    ...
    _push!(
        "Convolved Gamma+Uniform closed-form batched cdf",
        (θ, xs) -> sum(
    ...
    _push!(
        "Convolved Gamma+Uniform closed-form logpdf",
        (θ, xs) -> sum(
            x -> logpdf(
                convolved(Gamma(θ[1], θ[2]), Uniform(0.0, 2.0)), x
            ),
    ...
    _push!(
        "Convolved Normal+Uniform closed-form pdf",
        (θ, xs) -> sum(
            x -> pdf(
    ...
    _push!(
        "Difference Normal-Normal analytical",
        (θ, obs) -> sum(
            z -> logpdf(
                difference(
                    Normal(θ[1], θ[2]), Normal(0.0, 1.0)
    ...
    _push!(
        "Difference Gamma-LogNormal numerical wrt X",
        (θ, obs) -> sum(
            z -> logpdf(
                difference(
                    Gamma(θ[1], θ[2]), LogNormal(0.5, 0.4)
    ...
    _push!(
        "Difference LogNormal-Gamma numerical wrt Y",
        (θ, obs) -> sum(
            z -> logpdf(
                difference(
                    LogNormal(0.5, 0.4), Gamma(θ[1], θ[2])
    ...
    _push!(
        "Difference Gamma-Normal mean+var moments",
    ...
    _push!(
        "Product LogNormal*LogNormal analytical",
        (θ, obs) -> sum(
            z -> logpdf(
                product(
                    LogNormal(θ[1], θ[2]), LogNormal(0.5, 0.4)
                ), z
            ), obs
        ),
    ...
    _push!(
        "Product Gamma*LogNormal numerical wrt X",
        (θ, obs) -> sum(
            z -> logpdf(
                product(
                    Gamma(θ[1], θ[2]), LogNormal(0.5, 0.4)
    ...
    _push!(
        "Product LogNormal*Gamma numerical wrt Y",
        (θ, obs) -> sum(
            z -> logpdf(
                product(
                    LogNormal(0.5, 0.4), Gamma(θ[1], θ[2])
    ...
    _push!(
        "Product Gamma*LogNormal mean+var moments",
    ...
    _push!(
        "Convolved Poisson+Geometric lattice",
        (θ, ks) -> sum(
    ...
    _push!(
        "Convolved NegativeBinomial+Poisson lattice (#85)",
        (θ, ks) -> sum(
            k -> logpdf(
    ...
    _push!(
        "Convolved Poisson+Poisson analytical (#89)",
        (θ, ks) -> sum(
    ...
    _push!(
        "Difference Poisson-Geometric lattice",
        (θ, ks) -> sum(
    ...
    _push!(
        "Product Poisson*Poisson lattice",
        (θ, ks) -> sum(
    ...
    _push!(
        "Convolved Poisson+Normal mixed fold (#115)",
        (θ, xs) -> sum(
    ...
    _push!(
        "Ratio Normal/Normal analytical",
        (θ, obs) -> sum(
            z -> logpdf(
                ratio(
                    Normal(0.0, θ[1]), Normal(0.0, θ[2])
                ), z
            ), obs
        ),
    ...
    _push!(
        "Ratio Gamma/LogNormal numerical wrt X",
        (θ, obs) -> sum(
            z -> logpdf(
                ratio(
                    Gamma(θ[1], θ[2]), LogNormal(0.5, 0.4)
    ...
    _push!(
        "Ratio LogNormal/Gamma numerical wrt Y",
        (θ, obs) -> sum(
            z -> logpdf(
                ratio(
                    LogNormal(0.5, 0.4), Gamma(θ[1], θ[2])
    ...
    _push!(
        "Ratio Gamma/Gamma analytic moments",
        (θ, _obs) -> let d = ratio(
    ...
    _push!(
        "Timeseries convolve discrete Poisson delay",
    ...
    _push!(
        "Timeseries convolve time-varying Poisson delays",
        (θ, s) -> sum(
            convolve_series(
    ...
    _push!(
        "Timeseries convolve time-varying PMF matrix",
        (θ, s) -> sum(
            convolve_series(
    ...
    _push!(
        "pgf Poisson closed form wrt rate",
