...
On a non-unit or shifted lag grid, pass a `Distributions.DiscreteNonParametric` instead of a bare vector — its support carries the grid — which matches the discrete-delay type ModifiedDistributions.jl uses:
