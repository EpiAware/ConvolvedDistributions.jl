...
`scaffold` seeds a starting set of docs source pages for `ConvolvedDistributions`
once; after that they belong to `ConvolvedDistributions`, not to the kit.
...
- Replace the seeded quickstart in `getting-started/index.md` with
  `ConvolvedDistributions`'s real installation steps and a runnable first example.
...
- The API reference pages, generated fresh from `ConvolvedDistributions`'s
