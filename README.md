...
- **Timeseries convolution**: `convolve_distributions(delay, series)` convolves a numeric series (e.g. expected infections over time) with the discretised delay PMF, giving expected downstream counts — the renewal-style observation layer — with gradients flowing through the delay parameters.
