...
# `UnivariateDistribution`s (the `_MixedableDifference` alias's own
# bound), so a duck-typed pair, which quadrature evaluates, never
# reports the fold. `Difference` always has exactly two components
# (unlike `Convolved`), so no arity guard is needed.
...
Routes through [`difference_quantile`](@ref), so a registered analytic
pair (currently `Normal`-`Normal`) is answered exactly with no
...
function quantile(d::Difference, p::Real)
    _validate_quantile_p(p)
    return difference_quantile(d, (d.x, d.y), p, d.method)
end
