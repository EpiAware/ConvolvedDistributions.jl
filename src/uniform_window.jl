...
# (#166): `ccdf_ad_safe(::Gamma)` is `1 - _gamma_cdf(...)`, which loses
# accuracy in exactly the far right tail this recursion exists to stay
# accurate in, while `logccdf_ad_safe(::Gamma)` computes the survival
# directly and stays accurate there (EpiAwareADTools#47).
